...
flag=0
class Performance:
    def __init__(self, employee_id: int, behaviour: int, team_collaboration: int, verbal_skills: int, critical_thinking: int, emotional_intelligence: int):
    ...
    def save_to_csv(self, file_path='employee_performance_data.csv'):
        ...
        with open(file_path, mode='w', newline='') as file:
            fieldnames = ['employee_id', 'behaviour', 'team_collaboration', 'verbal_skills', 'critical_thinking', 'emotional_intelligence', 'average']
...
with open(emp_file_path, mode='r', newline='') as file:
    ...
    for row in reader:
        if int(row['employee_id']) == employee_id:
            ...
            employee = Performance(employee_id, behaviour, team_collaboration, verbal_skills, critical_thinking, emotional_intelligence)
